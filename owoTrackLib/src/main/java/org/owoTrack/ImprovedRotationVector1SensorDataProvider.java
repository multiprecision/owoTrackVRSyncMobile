...
import org.owoTrack.math.MatrixF4x4;
 ...
 *
 */
public class ImprovedRotationVector1SensorDataProvider implements SensorDataProvider {
    ...
    ImprovedRotationVector1SensorDataProvider(SensorManager manager, UdpPacketHandler udpClient_v, AppStatus logger) throws Exception {
        ...
        if (gyroscopeSensor == null) {
            logger.update("Could not find any calibrated gyroscope sensor.");
            throw new Exception("Your device does not have the required sensors and is unsupported.");
        }
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            if (timestamp != 0) {
                ...
                if (Math.abs(dotProd) < OUTLIER_THRESHOLD) {
                    ...
                    setOrientationQuaternion(interpolatedQuaternion);
                ...
                if (panicCounter > PANIC_THRESHOLD) {
                    ...
                    if (gyroscopeRotationVelocity < 3) {
                        ...
                        setOrientationQuaternion(quaternionRotationVector);
            ...
            udpClient.sendRotationData(currentOrientationQuaternion);
